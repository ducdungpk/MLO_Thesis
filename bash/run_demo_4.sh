...
do
  ./ns3 run demo-4-multi-sta-dual-link.cc -- --nSta=5 --offeredLoad=$load \
  | tee /dev/tty \
  | grep "^RESULT," \
...
do
  ./ns3 run demo-4-multi-sta-dual-link.cc -- --nSta=$sta --offeredLoad=2.5 \
  | tee /dev/tty \
  | grep "^RESULT," \
...
do
  ...
  do
    ./ns3 run demo-4-multi-sta-dual-link.cc -- --nSta=$sta --offeredLoad=$load \
    | tee /dev/tty \
    | grep "^RESULT," \
